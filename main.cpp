...
void process_command_options(int argc, char *argv[])
{
    ...
    /* pass 2 — open input file and output files */
    for (int i = 1; i < argc; i++) {
        ...
        if (show_tokens) {
            char fname[512]; sprintf(fname, "%s.toks", argv[i]);
            tok_file = fopen(fname, "w");
        }
        if (show_ast && !sa_parse) {
            char fname[512]; sprintf(fname, "%s.ast", argv[i]);
            ast_file = fopen(fname, "w");
        }
        if (show_tac && !sa_parse && !sa_ast) {
            char fname[512]; sprintf(fname, "%s.tac", argv[i]);
            tac_file = fopen(fname, "w");
        }
        if (show_rtl && !sa_parse && !sa_ast && !sa_tac) {
            char fname[512]; sprintf(fname, "%s.rtl", argv[i]);
            rtl_file = fopen(fname, "w");
        }
        if (show_asm && !sa_parse && !sa_ast && !sa_tac && !sa_rtl) {
            char fname[512]; sprintf(fname, "%s.spim", argv[i]);
            asm_file = fopen(fname, "w");
        }
        break;   /* only one input file */
    }
}
...
int main(int argc, char *argv[])
{
    ...
    /* tokens — independent of pipeline stage */
    if (show_tokens && tok_file) {
        char *buf=NULL; size_t sz=0;
        FILE *mem=open_memstream(&buf,&sz);
        print_toks(mem); fclose(mem);
        fprintf(tok_file,"%s",buf); free(buf);
        fclose(tok_file);
    }

    if (sa_parse) return status;
    ...
    if (sa_ast) return status;

    /* TAC */
    if (show_tac && tac_file) {
        char *buf=NULL; size_t sz=0;
        FILE *mem=open_memstream(&buf,&sz);
    ...
    if (sa_rtl) return status;

    /* ASM */
    if (show_asm && asm_file) {
        char *buf=NULL; size_t sz=0;
        FILE *mem=open_memstream(&buf,&sz);
        print_asm(mem); fclose(mem);
        fprintf(asm_file,"%s",buf); free(buf);
        fclose(asm_file);
    }
